    ...
    def decorator(f):
        def inner(*args, **kwargs):
            ...
            logging.error(f"Failed to  {f.__name__} in " 
                          f"{attempts} attempts")
            if raise_error:
                raise rq.HTTPError(f"Failed to execute {f.__name__} "
                                   f"in {attempts} attempts due to "
                                   f"Error {e}")
        return inner
    return decorator
                ...
                str
                    ...
                    str
                        ...
                        float
                            ...
                            longitude,
                            float)):
            raise ValueError(f"Mixed scheme provided, provide latitude "
                             f"and longitude either in sign or compass "
                             f"format.")
